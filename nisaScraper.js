...
// FAT DONE INEO
// Fungsi untuk mengambil tiket Fat Loss yang berstatus DONE
async function scrapeFatLossDone() {
    const jwtToken = process.env.NISA_TOKEN;
    if (!jwtToken) return { success: false, error: 'Token NISA belum diisi di file .env!' };
    ...
    try {
        ...
        // Menggunakan endpoint fatloss-assignment-done sesuai data yang kamu berikan
        const response = await axios.get('https://nisa.myrepublic.net.id/api/transaction/fatlossticketing/fatloss-assignment-done', {
            params: {
                '_dc': dcParam,
                'wo_number': '',
                'vendor_name': '',
                ...
                'status_fat': '0',
                'workplace': '8',
                'userGroup': 'MSFO,Massproblem Only',
                'page': '1',
                'start': '0',
                'limit': '25'
            },
        ...
        if (resData && resData.success === true) {
            ...
            if (tickets.length === 0) {
                return { success: true, data: 'ℹ️ *Tidak ada riwayat tiket Fat Loss berstatus DONE baru-baru ini.*' };
            }

            let laporan = `✅ *DAFTAR TIKET FAT LOSS (DONE)* ✅\n`;
            laporan += `==================================\n\n`;

            // Tampilkan maksimal 10 data teratas agar chat tidak terlalu panjang
            const maxDisplay = Math.min(tickets.length, 10);
            
            for (let i = 0; i < maxDisplay; i++) {
                const tk = tickets[i];
                laporan += `${i + 1}. 📌 *No FT:* \`${tk.tlop_no || '-'}\`\n`;
                laporan += `   📍 *Cluster:* ${tk.tlop_cluster_name || '-'}\n`;
                laporan += `   🔌 *FDT/FAT:* \`${tk.tlop_fdt_code || '-'}\` / \`${tk.tlop_fat_code || '-'}\`\n`;
                laporan += `   👥 *Total CID:* \`${tk.total_cid || '0'}\` User\n`;
                laporan += `   👨‍🔧 *Confirm By:* ${tk.tlop_confirm_by || '-'}\n`;
                laporan += `   ⏰ *Tanggal Done:* ${tk.tlop_confirm_date || '-'}\n`;
                laporan += `   📝 *Hasil Kerja:* _${tk.tlos_remark_result || 'Tidak ada keterangan'}_\n`;
                
                // Info jika verifikasi tertolak/butuh verifikasi lanjutan
                if (tk.tlop_reason_unverify) {
                    laporan += `   ⚠️ *Catatan Verifikasi:* _${tk.tlop_reason_unverify}_\n`;
                }
                laporan += `   ----------------------------------\n`;
            }
            ...
            return { success: false, error: resData.msg || 'Gagal mengambil data dari API Fat Loss Done.' };
        }
    } catch (error) {
        return { success: false, error: error.message };
    }
...
// 5. MONITORING GANGGUAN PRE OPEN 
async function scrapePreOpen() {
    const jwtToken = process.env.NISA_TOKEN;
    if (!jwtToken) return { success: false, error: 'Token NISA belum diisi di file .env!' };
    ...
    try {
        ...
        const response = await axios.get('https://nisa.myrepublic.net.id/api/transaction/massproblem/getdata', {
            params: {
                '_dc': dcParam, 'tmas_area_id': '', 'tmas_massproblem_no': '', 'tmas_start_date': '', 'tmas_end_date': '',
                'tmas_category': '', 'tmas_category_problem': '', 'ftmad_cid': '', 'bxmq_clusters': '', 'bxmq_hostname': '',
                'tmas_status': '1', 'trxType': '1', 
                'tmas_is_pre_massproblem': '1', // <-- KUNCINYA DI SINI: Diubah ke '1' untuk menarik Pre-MP Open
                'page': '1', 'start': '0', 'limit': '150'
            },
            headers: getBaseHeaders(jwtToken)
        });
        ...
        const resData = response.data;
        if (resData && resData.success === true) {
            const listPreGamas = resData.data || [];
            if (listPreGamas.length === 0) {
                return { success: true, data: `🟢 *MONITORING PRE-MP MASS PROBLEM*\n\nStatus: *Aman*\nAlhamdulillah, tidak terdeteksi adanya Pre-MP Open aktif saat ini.` };
            }

            let laporan = `📌 *MONITORING DATA PRE-MP OPEN (NISA)* 📌\n`;
            laporan += `==================================\n\n`;

            const maxDisplay = Math.min(listPreGamas.length, 8);
            for (let i = 0; i < maxDisplay; i++) {
                const olt = listPreGamas[i];
                laporan += `${i + 1}. 🎫 *Pre-MP No:* \`${olt.tmas_massproblem_no || '-'}\`\n`;
                laporan += `   🖥️ *Host Name:* \`${olt.tmas_link_dtl || '-'}\`\n`;
                laporan += `   📍 *Area:* ${olt.tmas_area_name || '-'}\n`;
                laporan += `   📊 *Status:* ⚠️ PRE-OPEN\n`;
                laporan += `   🔀 *Type:* ${olt.tmas_type || '-'} (${olt.tmas_remark_name || '-'})\n`;
                laporan += `   📁 *Category:* ${olt.tmas_category_name || '-'}\n`;
                laporan += `   ⚙️ *Problem Category:* ${olt.tmas_category_type_name || '-'}\n`;
                laporan += `   💥 *NE Impact:* ${olt.tmas_ne_name || '-'}\n`;
                ...
                laporan += `   📅 *Start Date:* ${olt.tmas_start_date || '-'}\n`;
                laporan += `   ⏰ *Start Time:* ${olt.tmas_start_time || '-'}\n`;
                laporan += `   💡 _Ketik *\.cekhost ${olt.tmas_id || ''}* untuk rincian detail OLT_\n`; // <-- DIUBAH JADI LEBIH SINGKAT
                laporan += `   ----------------------------------\n`;
            }
            return { success: true, data: laporan };
        } else {
            return { success: false, error: 'Gagal mengambil data list Pre-MP Open.' };
        }
    } catch (error) {
        return { success: false, error: error.message };
    }
...
// FUNGSI SCRAPE OTOMATIS PRE OPEN SEMARANG 
async function scrapePreMpSemarang() {
    const jwtToken = process.env.NISA_TOKEN;
    if (!jwtToken) return { success: false, error: 'Token NISA belum diisi di file .env!' };
    try {
        const dcParam = Date.now();
        const response = await axios.get('https://nisa.myrepublic.net.id/api/transaction/massproblem/getdata', {
            params: {
                '_dc': dcParam, 'tmas_area_id': '', 'tmas_massproblem_no': '', 'tmas_start_date': '', 'tmas_end_date': '',
                'tmas_category': '', 'tmas_category_problem': '', 'ftmad_cid': '', 'bxmq_clusters': '', 'bxmq_hostname': '',
                'tmas_status': '1', 'trxType': '1', 'tmas_is_pre_massproblem': '1', 'page': '1', 'start': '0', 'limit': '150'
            },
            headers: getBaseHeaders(jwtToken)
        });
        const resData = response.data;
        if (resData && resData.success === true) {
            const listAllPreGamas = resData.data || [];
            const listPreGamasSemarang = listAllPreGamas.filter(olt => {
                return olt.tmas_area_name && olt.tmas_area_name.toUpperCase() === 'SEMARANG';
            });
            if (listPreGamasSemarang.length === 0) return { success: true, data: '' }; 
            let laporan = "";
            const maxDisplay = Math.min(listPreGamasSemarang.length, 5);
            for (let i = 0; i < maxDisplay; i++) {
                const olt = listPreGamasSemarang[i];
                laporan += `🎫 *Pre-MP No:* \`${olt.tmas_massproblem_no || '-'}\`\n`;
                laporan += `🖥️ *Host Name:* \`${olt.tmas_link_dtl || '-'}\`\n`;
                laporan += `🔀 *Type:* ${olt.tmas_type || '-'} (${olt.tmas_remark_name || '-'})\n`;
                laporan += `📁 *Category:* ${olt.tmas_category_name || '-'}\n`;
                laporan += `⚙️ *Problem Category:* ${olt.tmas_category_type_name || '-'}\n`;
                laporan += `👥 *Impacted CID:* ${olt.tmas_tot_cid || '0'} User\n`;
                laporan += `🕒 *Aging Ticket:* ${olt.tmas_aging || '-'}\n`;
                laporan += `📅 *Waktu Mulai:* ${olt.tmas_start_date || '-'} pukul ${olt.tmas_start_time || '-'}\n`;
                laporan += `💡 _Ketik *\.cekhost ${olt.tmas_id || ''}* untuk rincian detail OLT_\n`;
                if (i < maxDisplay - 1) laporan += `----------------------------------\n`;
            }
            return { success: true, data: laporan };
        } else {
            return { success: false, error: 'Gagal memproses data API Pre-MP Open.' };
        }
    } catch (error) {
        return { success: false, error: error.message };
    }
...
// 6. CEK PRE OPEN VIA HOSTNAME OLT
async function cekDetailHost(tmasId) {
    const jwtToken = process.env.NISA_TOKEN;
    if (!jwtToken) return { success: false, error: 'Token NISA belum diisi di file .env!' };
    ...
    try {
        ...
        const response = await axios.get('https://nisa.myrepublic.net.id/api/transaction/massproblem/massproblemdportdtl', {
            params: {
                '_dc': dcParam,
                'tmas_id': tmasId.trim(),
                'tmas_device_name': '' // <-- Sengaja dikosongkan karena NISA sudah bisa membaca lewat tmas_id saja
            },
            headers: getBaseHeaders(jwtToken)
        });
        ...
        if (resData && resData.success === true) {
            ...
            if (arrData.length === 0) {
                return { success: true, data: `🖥️ *DETAIL PRE-MP OLT*\n\nID: \`${tmasId}\`\nData rincian port kosong atau tidak ditemukan.` };
            }

            const info = arrData[0];
            
            let hasilTeks = `🖥️ *DETAIL STATUS PRE-MP OLT* 🖥️\n`;
            hasilTeks += `==================================\n\n`;
            hasilTeks += `🏢 *OLT Name:* \`${info.tmad_olt_name || '-'}\`\n`;
            hasilTeks += `🎛️ *Position:* Frame ${info.tmad_frame || '0'} / Slot ${info.tmad_slot || '-'} / Port ${info.tmad_port || '-'}\n`;
            hasilTeks += `📡 *Alarm ID:* \`${info.tmad_alarm_id || '-'}\`\n`;
            hasilTeks += `📍 *Cluster:* ${info.tmad_cluster_name || '-'}\n`;
            hasilTeks += `⏰ *Alert Happen:* ${info.tmad_alert_happentime || '-'}\n`;
            hasilTeks += `📊 *Status Port:* ${info.tmad_status_port === '1' ? '🔴 DOWN / ALERT' : '🟢 NORMAL'}\n`;
            
            return { success: true, data: hasilTeks };
        } else {
            return { success: false, error: resData.msg || `Gagal membongkar rincian port untuk ID ${tmasId}` };
        }
    } catch (error) {
        return { success: false, error: error.message };
    }
...
// 7. MONITORING GANGGUAN MASSAL AREA SEMARANG ONLY
...
// 8. MONITORING PRE-MASSIVE PROBLEM AREA SEMARANG ONLY
async function scrapePreMpSemarang() {
    const jwtToken = process.env.NISA_TOKEN;
    if (!jwtToken) return { success: false, error: 'Token NISA belum diisi di file .env!' };
    ...
    try {
        ...
        const response = await axios.get('https://nisa.myrepublic.net.id/api/transaction/massproblem/getdata', {
            params: {
                ...
                'tmas_category': '', 'tmas_category_problem': '', 'ftmad_cid': '', 'bxmq_clusters': '', 'bxmq_hostname': '',
                'tmas_status': '1', 'trxType': '1', 
                'tmas_is_pre_massproblem': '1', // 🔥 KUNCI: Diubah ke '1' untuk mengambil PRE-MP
                'page': '1', 'start': '0', 'limit': '150'
            },
            headers: getBaseHeaders(jwtToken)
        });
        ...
        const resData = response.data;
        if (resData && resData.success === true) {
            const listPreMp = resData.data || [];
            
            // Filter hanya untuk area SEMARANG saja
            const dataSemarang = listPreMp.filter(olt => olt.tmas_area_name && olt.tmas_area_name.toUpperCase() === 'SEMARANG');

            if (dataSemarang.length === 0) {
                return { success: true, data: `🟢 *MONITORING PRE-MP OPEN - AREA SEMARANG*\n\nStatus: *Aman*\nAlhamdulillah, tidak ada indikasi Pre-Massive Problem (Pre-MP Open) di area Semarang saat ini.` };
            }

            let laporan = `🚨 *MONITORING PRE-MASSIVE PROBLEM OPEN - SEMARANG* 🚨\n`;
            laporan += `==================================\n\n`;

            dataSemarang.forEach((olt, idx) => {
                const idTmas = olt.tmas_id || '-'; // ID_Tmas Angka Murni
                const hostName = olt.tmas_link_dtl || '-';
                const clusters = olt.bxmq_clusters || '-';
                const statusNama = '🔴 PRE-MP OPEN';
                const agingTicket = olt.tmas_aging || '-';
                const impactedCid = olt.tmas_tot_cid || '0';
                const area = olt.tmas_area_name || '-';
                const category = olt.tmas_category_name || '-';
                ...
                laporan += `${idx + 1}. 📄 *ID Tmas:* \`${idTmas}\`\n`;
                laporan += `   🖥️ *Host Name:* \`${hostName}\`\n`;
                laporan += `   📍 *Cluster:* \`${clusters}\`\n`;
                laporan += `   📊 *Status:* ${statusNama} (${detailProgress})\n`;
                laporan += `   📁 *Category:* ${category}\n`;
                laporan += `   ⚙️ *Problem Category:* ${probCategory}\n`;
                laporan += `   💥 *NE Impact:* ${neImpact}\n`;
                laporan += `   📺 *Service Impact:* ${serviceImpact}\n`;
                laporan += `   👥 *Impacted CID:* ${impactedCid} User\n`;
                laporan += `   🕒 *Aging Ticket:* ${agingTicket}\n`;
                laporan += `   📅 *Start Date:* ${startDate} ${startTime}\n`;
                laporan += `   👤 *Created by:* ${createdBy}\n`;
                laporan += `   ----------------------------------\n`;
            });
            ...
            return { success: false, error: 'Gagal mengambil data list Pre-MP.' };
        }
    } catch (error) {
        return { success: false, error: error.message };
    }
